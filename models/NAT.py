...
class NAT(torch.nn.Module):
    def __init__(self, n_feat, e_feat, memory_dim, total_nodes, get_checkpoint_path=None, get_ngh_store_path=None,
                 get_self_rep_path=None, get_prev_raw_path=None, time_dim=2, pos_dim=0, n_head=4,
                 num_neighbors=['1', '32'],
                 dropout=0.1, linear_out=False, verbosity=1, seed=1, n_hops=2, replace_prob=0.9, self_dim=100,
                 ngh_dim=8, device=None):
        ...
        self.affinity_score = MergeLayer(self.feat_dim + self_dim, self.feat_dim + self_dim, self.feat_dim + self_dim,
                                         1,
                                         non_linear=not self.linear_out)  # torch.nn.Bilinear(self.feat_dim, self.feat_dim, 1, bias=True)
    ...
    def backup_memory_bank(self):
        ...
        for i in range(len(self.num_neighbors)):
            clone_ngh_stores.append(self.neighborhood_store[i].data.clone())
        return clone_ngh_stores, self.self_rep.data.clone(), self.prev_raw.data.clone()

    def reload_memory_bank(self, backup_memory_bank):
        for i in range(len(self.num_neighbors)):
            self.neighborhood_store[i].data = backup_memory_bank[0][i].clone()
        self.self_rep.data = backup_memory_bank[1].clone()
        self.prev_raw.data = backup_memory_bank[2].clone()
    ...
    def position_bits(self, bs, hop):
        ...
        return torch.ones(bs * self.num_neighbors[hop], device=self.device).long() << hop

    def compute_src_dst_node_temporal_embeddings(self, src_l_cut, tgt_l_cut, cut_time_l, e_idx_l=None, update=True):
        start = time.time()
        start_t = time.time()
        ...
        idx_th = torch.cat((src_th, tgt_th), 0)
        ...
        if self.n_hops > 1:
            ...
            updated_mem_h2 = torch.cat(
                (self.batch_fetch_ncaches(idx_th, cut_time_th.repeat(2), hop=2), h2_pos_bit.unsqueeze(1)), -1)
        ...
        tgt_pos_raw = pos_raw[src_nghs:src_nghs + tgt_nghs].long() << 3
        ...
        features = joint_p#torch.cat((joint_p), 0)

        src_self_rep = self.updated_self_rep(src_th)
        tgt_self_rep = self.updated_self_rep(tgt_th)

        p_score, attn_score = self.forward(ngh_and_batch_id_p, features, batch_size,
    ...
    def forward(self, ngh_and_batch_id_p, feat, bs, src_self_rep=None, tgt_self_rep=None):
        edge_idx = ngh_and_batch_id_p.T#torch.cat((ngh_and_batch_id_p), dim=0).T
        embed, _, attn_score = self.gat((feat, edge_idx.long(), bs))
        p_embed = embed[:bs]
